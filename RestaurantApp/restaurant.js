...
function getform() {
    ...
    sub.onclick = function () {
        console.log('a')
        if (username.value == "" || email.value == "" || date.value == "" || time.value == "" || number.value == "") {
            alert("Please fill all the details")
        }
        else if (!email.value.includes('@') || !email.value.includes('.')) {
            alert('Invalid Email!!')
        }
        else if (number.value.toString().length !== 10) {
            alert('Please enter valid mobile number!!')
        }
        else {
            let obj = { username: username.value, email: email.value, date: date.value, time: time.value, no_of_peoples: nos.value, number: number.value }
            ...
            li.onclick = function () {
                ...
                contli.innerHTML = (`<br><b>Email:</b>  ${obj.email}<br><b>Reservation Date:</b>  ${dateCal(obj.date)}<br><b>Reservation time:</b>  ${obj.time}<br><b>No.of.peoples:</b>  ${obj.no_of_peoples}<b><br/>Mobile number:</b>  ${obj.number}`)
